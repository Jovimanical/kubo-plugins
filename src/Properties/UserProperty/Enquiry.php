...
class Enquiry {
    public static function newEnquiry(array $data){
        ...
        $result = DBQueryFactory::insert("[Properties].[Enquiry]", $inputData, false);

        return $result;
    }

    public static function viewProperties(int $userId){
        $query = "SELECT a.* FROM Properties.Enquiry a INNER JOIN SpatialEntities.Entities b ON a.LinkedEntity = b.EntityId WHERE a.UserId = $userId AND b.EntityParent IS NULL";
    ...
    public static function viewEnquiry(int $EnquiryId){
        $query = "SELECT * FROM Properties.Enquiry WHERE EnquiryId = $EnquiryId";
        $result = DBConnectionFactory::getConnection()->query($query)->fetchAll(\PDO::FETCH_ASSOC);
    ...
    public static function viewEnquiryByName(array $data){
        $name = $data["name"] ?? 0;
        $query = "SELECT * FROM Properties.Enquiry WHERE EnquiryTitle = '$name'";
        $result = DBConnectionFactory::getConnection()->query($query)->fetchAll(\PDO::FETCH_ASSOC);
    ...
    public static function viewEnquiryChildren(int $EnquiryId){
        $query = "SELECT a.*, b.EntityParent FROM Properties.Enquiry a INNER JOIN SpatialEntities.Entities b ON a.LinkedEntity = b.EntityId WHERE b.EntityParent = (SELECT LinkedEntity FROM Properties.Enquiry WHERE EnquiryId = $EnquiryId)";
    ...
    public static function viewEnquiryChildrenMetadata(int $parentId){
        $query = "SELECT a.* FROM Properties.EnquiryMetadata a 
                    INNER JOIN Properties.Enquiry b ON a.EnquiryId = b.EnquiryId
