...
class UserProperty
{
    ...
    public static function newPropertyEstateLong(array $data)
    {
        ...
        if (count($selectExec) > 1) {
            ...
            $updateQuery = "UPDATE Properties.MapDataUploadStata SET UploadStatus = 'processing' WHERE UserId = $user AND FolderName = '$title' AND Initials = '$initials'";
            $updateExec = DBConnectionFactory::getConnection()->exec($updateQuery);
        } else {
            // progress check
            $insertQuery = "INSERT INTO Properties.MapDataUploadStata (UserId,FolderName,Initials,UploadStatus) VALUES ($user,'$title','$initials','processing')";
    ...
    public static function listAllProperties(int $propertyId = 1, array $data)
    {
        ...
        foreach ($results as $key => $result) {
            ...
            $blockQueries[] = "SELECT a.MetadataId, a.FieldName, a.FieldValue, a.PropertyId FROM Properties.UserPropertyMetadata a INNER JOIN Properties.UserProperty b ON a.PropertyId = b.PropertyId  WHERE a.PropertyId = $resultPropertyId AND b.PropertyFloor = $resultPropertyFloor";
    ...
    public static function searchByLocation(int $userId, array $data)
    {
        ...
        $latFrom = $data['latUser'] ?? null;
        $lngFrom = $data['lngUser'] ?? null;
